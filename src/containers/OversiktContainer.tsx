...
const mapStateToProps = ({ personoversikt, personregister, veilederenheter, veilederinfo }: ApplicationState) => ({
  personregister,
...
const OversiktCont = ({type}: OversiktProps) => {

  const [ hendelseTypeFilter, onHendelsesTypeChange ] = useState({
    onskerMote: true,
    svartMote: true,
    ufordeltBruker: true,
  });
  ...
  const stateProps = useSelector((state: ApplicationState) => state);
  const {
    henterAlt,
    noeErHentet,
    altFeilet,
  ...
  useEffect(() => {
    dispatch(actions.hentVeilederenheter());
  }, []);

  useEffect(() => {
    dispatch(actions.hentPersonoversiktForespurt());
  }, [personregister.hentet]);
